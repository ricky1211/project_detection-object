...
// ... (Seluruh kode DetectionToolkit yang Anda berikan) ...
...
const DetectionToolkit = () => {
  ...
  // ... (Sisa fungsi-fungsi lainnya: handleImageUpload, startWebcam, stopWebcam, 
  //     startContinuousDetection, drawDetections, updateStats, saveToHistory, 
  //     downloadResults, exportData) ...
  ...
  const startContinuousDetection = (width, height) => {
    ...
    detectionIntervalRef.current = setInterval(async () => {
      if (videoRef.current && canvasRef.current && videoRef.current.readyState >= 2) {
        ...
        // The canvas size is already set in startWebcam
        // Ensure it draws the video frame accurately
  ...
  const displayStyle = imageUrl || isDetecting ? {} : { 
    // Ensure the canvas takes up space even when empty for better layout
                  ...
                  {detectionHistory.map((item) => (
                    <div key={item.id} className="bg-gray-700 rounded px-4 py-3 border border-gray-600 hover:bg-gray-600 transition">
                      <div className="flex justify-between items-start">
                        ...
                        {/* You could add a button here to reload the image/detection */}
                      </div>
                    </div>
                  ))}
                </div>
